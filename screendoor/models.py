...
class Position(models.Model):
    ...
    def update_applicant_fields(self):
        if self.applicant_set.all().count() > 0:
            ...
            self.mean_score = sum([
                FormAnswer.objects.filter(parent_applicant=applicant,
                                          applicant_answer=True).count() *
                100 //
                FormAnswer.objects.filter(parent_applicant=applicant).count()
...
class Applicant(models.Model):
    parent_position = models.ForeignKey(Position,
                                        on_delete=models.CASCADE,
                                        null=True)
    ...
    LANGUAGE_PROFICIENCY_CHOICES = [('NA', 'None'), ('BEG', 'Beginner'),
                                    ('INT', 'Intermediate'),
                                    ('ADV', 'Advanced')]
    ...
    english_working_ability = models.CharField(
        choices=LANGUAGE_PROFICIENCY_CHOICES, max_length=200, null=True)
    first_official_language = models.CharField(choices=LANGUAGE_CHOICES,
                                               max_length=200,
                                               null=True)
    written_exam = models.CharField(choices=LANGUAGE_CHOICES,
                                    max_length=200,
                                    null=True)
    correspondence = models.CharField(choices=LANGUAGE_CHOICES,
                                      max_length=200,
                                      null=True)
    interview = models.CharField(choices=LANGUAGE_CHOICES,
                                 max_length=200,
                                 null=True)
    ...
    def update_question_fields(self):
        ...
        self.classification_names = "".join([
            (classification.classification_substantive or "")
...
class RequirementMet(models.Model):
    parent_applicant = models.ForeignKey(Applicant,
                                         on_delete=models.CASCADE,
                                         null=True,
                                         related_name='requirementsmet')
...
class FormQuestion(models.Model):
    ...
    parent_requirement = models.ForeignKey(Requirement,
                                           on_delete=models.CASCADE,
                                           null=True,
                                           related_name='parent_req')
...
class FormAnswer(models.Model):
    parent_question = models.ForeignKey(FormQuestion,
                                        on_delete=models.CASCADE,
                                        null=True,
                                        related_name='answer')
    parent_applicant = models.ForeignKey(Applicant,
...
class Qualifier(models.Model):
    QUALIFIER_TYPES = [('RECENCY', 'Recency'),
                       ('SIGNIFICANCE', 'Significance')]
    RESULT = [('PASS', "Passed"), ('IND', "Indeterminate"), ('FAIL', "Fail")]

    parent_answer = models.ForeignKey(FormAnswer,
                                      on_delete=models.CASCADE,
                                      null=True,
                                      related_name='answers')
    qualifier_text = models.TextField(blank=True, null=True)
    qualifier_type = models.CharField(choices=QUALIFIER_TYPES,
                                      max_length=200,
                                      null=True)
    status = models.CharField(choices=RESULT, max_length=200, null=True)
...
class NlpExtract(models.Model):
    EXTRACT_TYPES = [('WHEN', 'A date or date range, and its context'),
                     ('HOW', 'What applicant did to fulfill a requirement')]
    parent_answer = models.ForeignKey(FormAnswer,
                                      on_delete=models.CASCADE,
                                      null=True,
                                      related_name='extract')
    extract_type = models.CharField(choices=EXTRACT_TYPES,
                                    max_length=200,
                                    null=True)
