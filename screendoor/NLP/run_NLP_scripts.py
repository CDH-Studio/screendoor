...
def generate_nlp_extracts(answer_text, linked_question, closing_date):
    # Create the NLP doc objects of both the originally formatted text,
    # and the secret reformatted text
    orig_doc = NLP_MODEL(answer_text)
    ...
    date_ranges = create_list_of_ranges(doc.ents, datetime.now())

    if linked_question is not None:
        analyze_question_qualifiers(linked_question, date_ranges, answer_text)
    ...
    if dates_and_contexts != [] and dates_and_contexts is not None:
        save_nlp_extracts(dates_and_contexts, 'WHEN', linked_question)
    ...
    if experiences != [] and experiences is not None:
        save_nlp_extracts(experiences, 'HOW', linked_question)
...
# timeframe (recency)
def analyze_question_qualifiers(question_text, date_ranges, answer):
    ...
    for sentence in nlp_question_text.sents:
        for date in [x for x in sentence.ents if 'DATE' in x.label_]:
            ...
            if re.search(r'least|over|more|approximately', date.text):
                sig_met = determine_if_significant_criteria_met(date_ranges, quantifier,
                                                      is_years)
                if sig_met == False:
                    print("SIGNFICANCE FAILED: ", question_text, answer)
            elif re.search(r'within|last', date.text):
                # replace with: closing date of position
                rec_met = determine_if_recent_criteria_met(date_ranges, datetime.today(),
                                                 quantifier, is_years)
                if rec_met == False:
                    print("RECENCY FAILED: ", question_text, answer)
